...
class RequestForBook(APIView):
    ...
    def post(self, request):
        ...
        book = ListedBooks.objects.filter(id=book_id, added_by_user_ids__contains=[
                                          issuer_user_id]).first()
        if book is None:
            return Response({"message": "Book Doesn't Exist.",
                             }, status=status.HTTP_400_BAD_REQUEST)

        if user_id in book.added_by_user_ids:
            user = User.objects.filter(id=user_id).first()
            if user is not None:
                # Check if book is borrowed previously or Not
                book_check = BooksRequested.objects.filter(
                    book_id=book_id, issuer_user_id=issuer_user_id).first()
